...
* data import, tidying and/or presentation 
...
.pull-left[
Top left Panel
* Script - Write, edit and save code and comments to keep  
...
* .RData: a environment file also know as a workspace. Objects but no code
...
.footnote[
<br>`r text_spec("Extra exercise:", background = "#25496b", color = "#f6fafd", bold = TRUE)`  Change the colour of the lines around each bar to black.]
--
...
Instead of having a population in each column, we often

have, **and want**, all measurements in one column with a second column giving the group. 
...
`geom_density()` can also be used when `distance` is mapped to `x` and  `y` gives counts.
